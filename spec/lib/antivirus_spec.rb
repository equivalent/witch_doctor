...
RSpec.describe WitchDoctor::Antivirus do
  ...
  context "when unchecked resource" do
    ...
    it { expect { subject.infected? } .to throw_symbol(:file_not_scaned) }
    it { expect { subject.clean? } .to throw_symbol(:file_not_scaned) }
    it { expect { subject.error? } .to throw_symbol(:file_not_scaned) }
